...
## IndigoOSを実機起動してPL UARTシェルへ接続

OSはHALTせずシェルで待機するため、通常の `make hardware-test` は使わない。書込み前に端末Aで受信端末を開く。

```bash
picocom -b 115200 --imap lfcrlf /dev/ttyUSB2
```

端末BでOSをBRAMへ組み込み、bitstream生成とJTAG書込みを行う。

```bash
make hardware-os-test
```
...
# 端末Aでpicocomを起動
vivado -mode batch -source scripts/program_os.tcl -nojournal -nolog
```

配線はUSB-UARTのTXをPMODA JA2（PL RX、Y19）、RXをPMODA JA1（PL TX、Y18）、GNDをGNDへ接続する。変換器は3.3 Vロジックに限定し、PYNQ-Z1のPS内蔵USB-UARTはこのPL UARTとは別系統である。起動後は `indigo> ` が表示され、`help`、`echo hello`、`ps`、`mem`、`uptime`、`cat /hello.txt`を入力できる。

RXは115200 baud、8N1、16 byte FIFOで、UART RX割り込みからカーネルのリングバッファへ移送する。CR/LF、Backspace、Deleteを処理する。RX未接続時はXDCの内部プルアップでアイドルHighを維持する。

外付けUSB-UART変換器や適切な配線がない場合、JTAGだけでPL UART RX入力を実機検証することはできない。その場合は `make simulate-os` と `make test-os-differential`で代替検証し、物理RXは未検証と記録する。
