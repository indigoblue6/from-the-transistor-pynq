...
if command -v iverilog >/dev/null 2>&1; then
    ...
    vvp build/cpu_tb.vvp "+MEM=$mem" "+EXPECT=$expected" "+MAX_CYCLES=$max_cycles" $output_arg $register_arg
    ...
    build/obj_cpu/cpu_tb "+MEM=$mem" "+EXPECT=$expected" "+MAX_CYCLES=$max_cycles" $output_arg $register_arg
